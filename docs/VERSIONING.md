...
- Current boundary: draft needs, proposals, shadow plans, fitness scores,
  ...
  records only; no patch application, Git, GitHub, file modification,
  autonomous activation, or runtime self-modification
- Next architecture target: Local GitConnector
...
Version `0.5.0` may create memories through the deterministic Reaction Core,
...
through GENUS itself, start agents/workers, call an LLM, run arbitrary shell
commands, or introduce vector/graph storage.
